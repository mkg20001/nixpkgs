...
Recent plan-ai commits to be aware of (check `git log mkg-patch..plan-ai` for the live list — these change): openclaw, kanbn, nexa, mcporter, ollama flavours, nix, hermes-agent.
...
**Flake-shim packages** (currently just `hermes-agent`) have a different bump procedure — see step 4.10 and skip 4.1–4.9 for them.
...
### 4.10 Bumping `hermes-agent` (flake-shim)

`pkgs/by-name/he/hermes-agent/package.nix` is a thin flake-compat shim — there is no `version` field, just a pinned `tag`, `rev`, and source `hash`. Transitive inputs (uv2nix, pyproject-nix, npm-lockfile-fix, flake-parts) are resolved automatically from the upstream `flake.lock` at the pinned tag, so you do not bump them by hand.

1. **Find the latest tag** on `NousResearch/hermes-agent`:
   ```bash
   gh release view --repo NousResearch/hermes-agent --json tagName -q '.tagName'
   ```
   Compare with the current `tag = "v..."` in `package.nix`. Skip if equal.
2. **Resolve the tag to a commit sha** (tags can be annotated, so dereference):
   ```bash
   gh api repos/NousResearch/hermes-agent/commits/<new-tag> --jq '.sha'
   ```
3. **Edit `package.nix`**: update `tag` and `rev`, then invalidate the hash:
   ```nix
   tag = "<new-tag>";
   rev = "<new-commit-sha>";
   hash = "";
   ```
4. **Build** and pick up the new source hash from the `got:` line:
   ```bash
   NIXPKGS_ALLOW_UNFREE=1 NIXPKGS_ALLOW_INSECURE=1 nix-build -A hermes-agent --out-link /tmp/result-hermes-agent 2>&1
   ```
   Set the reported hash and rebuild. The flake-compat layer will lazily fetch any new transitive inputs straight from the upstream `flake.lock` (verified via the lock's `narHash`), so you should not have to touch anything else. If a transitive input fails to fetch, inspect the new `flake.lock` and confirm the input type is one flake-compat handles (github/git/path/tarball/gitlab/sourcehut).
5. **Verify the binary**:
   ```bash
   /tmp/result-hermes-agent/bin/hermes --help
   ```
6. **Commit** with the tag (not a version number) since that is what is pinned:
   ```
   hermes-agent: <old-tag> -> <new-tag>
   ```
7. Skip the worktree / per-package branch step (4.9) — `hermes-agent` is a plan-ai-only package, it does not get upstreamed to `NixOS/nixpkgs`.
...
- `nix`
- `mcporter`
- `cudatoolkit`, `rocmPackages.rocm-smi` (Linux only)
- `lmstudio`
- `unsloth`
- `hermes-agent`
